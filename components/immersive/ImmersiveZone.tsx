  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-auto absolute top-[max(0.55rem,env(safe-area-inset-top))] right-[max(0.55rem,env(safe-area-inset-right))] z-[50] inline-grid w-max max-w-[min(100vw-1.3rem,22rem)] grid-cols-1 gap-1.5 justify-items-stretch sm:top-4 sm:right-4 sm:gap-2">
        ...
        <div className={`relative hidden sm:block ${GeistSans.className}`}>
          <div
      ...
      <div className={`pointer-events-auto absolute right-[max(0.55rem,env(safe-area-inset-right))] bottom-[max(0.75rem,env(safe-area-inset-bottom))] z-[2147483647] sm:hidden ${GeistSans.className}`}>
        <div
          className="flex h-9 min-w-0 w-full items-stretch rounded-none border shadow-sm"
          style={{ borderColor: immersiveBorderColor, backgroundColor: immersiveBackground }}
        >
          <button
            type="button"
            onClick={immersivePrevTrack}
            className="inline-flex h-9 w-9 shrink-0 cursor-pointer items-center justify-center border-0 bg-transparent text-xs font-semibold uppercase tracking-[0.08em] transition-colors hover:bg-muted"
            aria-label="Previous immersion track"
            title="Previous track"
            style={{ color: navLinkColor }}
          >
          ...
          <button
            type="button"
            onClick={toggleImmersiveMusicPaused}
            className="relative inline-flex h-9 min-w-0 flex-1 cursor-pointer items-center justify-center overflow-hidden rounded-none border-0 bg-transparent px-3 transition-colors hover:bg-muted"
            aria-pressed={immersiveMusicPaused}
            aria-label={
              immersiveMusicPaused ? "Resume immersion music" : "Pause immersion music"
            }
